<template>
  <div style="text-align: center; padding: 10px;">
    ...
    <div  id="albumMenu" data-html2canvas-ignore="true">
      <v-btn @click="appendArea">한칸 더 만들기</v-btn>
      <v-btn @click="exportToPDF">pdf 만들기 버튼</v-btn>
    ...
    <div id="contents" ref="pdfarea" >
      <div class="albumContent" v-for="(image, idx) in imageList" :key="idx" style="height: 100%; overflow: auto;">
        <div style="float: left; display: inline-block">
          <img :src="image" @click="dialogOn(idx)" style="width: 400px; height: 600px;"/>
          <textarea name="content" id="ucontent" rows="19" cols="70" placeholder="내용을 입력하세요" style="width: 400px; height: 100px; display: block;" :style="{ 'font-family': getMyFont.fontUrl }"/>
        </div>
      </div>
    </div>
    ...
    <v-dialog v-model="dialog">
      ...
      <div>
        ...
        <v-btn @click="getS3ImageList" style="width: 100%;">이미지 가져오기 버튼</v-btn>        
      </div>
...
export default {
  ...
  methods: {
      ...
      exportToPDF () {
          ...
          html2pdf(this.$refs.pdfarea, {
              ...
              html2canvas: { scrollY: 0, scale:1, dpi: 300, letterRendering: true, allowTaint: true , useCORS: true},
              jsPDF: {orientation: 'landscape', unit: 'mm', format: 'letter', compressPDF: true}
          })
      },
      getS3ImageList() {
        var selectDiaryId = -1 
  ...
  created() {
    ...
    let imageUrl = 'https://papers-bucket.s3.ap-northeast-2.amazonaws.com/store/diary-cover/%EB%85%B8%EC%9D%84%EC%BB%A4%EB%B2%84.jpg';
    fetch( imageUrl )
